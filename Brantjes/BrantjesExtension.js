export const BrantjesExtension = {
  ...
  render: ({ trace, element }) => {
    ...
    style.innerHTML = `
      .brantjes-carousel-container { position: relative; overflow: hidden; width: ${SLIDE_SIZE * VISIBLE}px; height: 420px; margin: auto; }
      .brantjes-carousel-track { display: flex; will-change: transform; height: 100%; transition: transform 0.6s cubic-bezier(0.16,1,0.3,1); }
      .brantjes-property-card { flex: 0 0 ${CARD_WIDTH}px; margin: 0 ${CARD_MARGIN/2}px; transition: transform 0.6s cubic-bezier(0.16,1,0.3,1), opacity 0.6s cubic-bezier(0.16,1,0.3,1); opacity: 0.6; transform: scale(0.85); position: relative; border-radius: 8px; background: #fff; box-shadow: 0 2px 6px rgba(0,0,0,0.1); cursor: pointer; overflow: hidden; }
      .brantjes-property-card.active { opacity: 1; transform: scale(1); }
      .brantjes-property-card-inner { position: relative; width: 100%; height: 100%; }
      .brantjes-property-card-inner img { width: 100%; height: 100%; object-fit: cover; }
      .brantjes-card-overlay { position: absolute; bottom: 0; left: 0; right: 0; padding: 15px; background: linear-gradient(to top, rgba(0,0,0,0.7), transparent); color: white; }
      .brantjes-card-info p { margin: 0 0 5px; }
      .brantjes-card-info p:first-child { font-weight: bold; }
      .brantjes-viewing-button { position: absolute; bottom: 10px; left: 50%; transform: translateX(-50%); background: #1E7FCB; color: white; border: none; padding: 8px 16px; border-radius: 4px; font-size: 12px; cursor: pointer; opacity: 0; transition: opacity 0.3s ease; pointer-events: none; }
      .brantjes-property-card.active .brantjes-viewing-button { opacity: 1; pointer-events: auto; }
      .brantjes-nav-button { position: absolute; top: 50%; transform: translateY(-50%); background: rgba(255,255,255,0.8); border: none; width: 40px; height: 40px; border-radius: 50%; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 24px; }
      .brantjes-nav-prev { left: 10px; }
      .brantjes-nav-next { right: 10px; }
      /* Modal Styles */
      .brantjes-modal-backdrop { position: fixed; inset: 0; background: rgba(0,0,0,0.6); display: flex; align-items: center; justify-content: center; opacity: 0; transition: opacity 0.3s ease; }
      .brantjes-modal-backdrop.visible { opacity: 1; }
      .brantjes-modal-container { background: white; border-radius: 8px; padding: 20px; max-width: 90%; max-height: 90%; overflow: auto; transform: scale(0.9); transition: transform 0.3s ease; }
      .brantjes-modal-backdrop.visible .brantjes-modal-container { transform: scale(1); }
      .brantjes-modal-close { position: absolute; top: 10px; right: 10px; background: none; border: none; font-size: 20px; cursor: pointer; color: #333; }
    `;
    ...
    slides[0].parentNode.parentNode.querySelector('.brantjes-nav-prev')?.addEventListener('click',()=>navigate(-1));
    slides[0].parentNode.parentNode.querySelector('.brantjes-nav-next')?.addEventListener('click',()=>navigate(1));
