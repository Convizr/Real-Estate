export const RealEstateExtension = {
  ...
  render: ({ trace, element }) => {
    ...
    // Global variables for booking flow.
    ...
    let personalEmail = "";
    let personalPhone = "";
    let selectedDateStr = ""; // selected date string for appointment

    // Booking breadcrumb globals.
    const bookingSteps = ["Property", "Appointment", "Personal Info", "Confirmation"];
    let visitedSteps = [true, false, false, false]; // initially, property is visited
    let bookingCurrentStep = 1; // start at Appointment view
    let clickableAreas = [];
    ...
    // --- Wrapper with consistent width (600px) ---
    ...
    // --- Helper Functions for Breadcrumb Drawing ---
    ...
    function renderBreadcrumb() {
      // Create or update a canvas at the top of container.
      let canvas = container.querySelector('#breadcrumbCanvas');
      if (!canvas) {
        canvas = document.createElement('canvas');
        canvas.id = 'breadcrumbCanvas';
        canvas.width = 600;
        canvas.height = 80;
        canvas.style.background = 'linear-gradient(to right, #5A46E3, #4A37D2)';
        ...
        canvas.addEventListener('click', function (event) {
          ...
          clickableAreas.forEach(area => {
            if (mouseX >= area.x && mouseX <= area.x + area.width &&
              ...
              // Allow navigation only to steps that are visited and are earlier than current step.
              if (visitedSteps[area.index] && area.index < bookingCurrentStep) {
                navigateToStep(area.index);
              }
            }
          });
        });
      ...
      // Define breadcrumb positions (4 steps).
      const breadcrumbs = [
        { label: 'Property', x: 20, width: 100 },
        { label: 'Appointment', x: 140, width: 120 },
        { label: 'Personal Info', x: 280, width: 140 },
        { label: 'Confirmation', x: 440, width: 100 }
      ];
      clickableAreas = [];
      breadcrumbs.forEach((crumb, index) => {
        // Use a fill color that indicates visited status.
        const fillColor = visitedSteps[index] ? '#5A46E3' : '#4A37D2';
        ctx.lineWidth = (index === bookingCurrentStep) ? 3 : 0;
        ctx.strokeStyle = (index === bookingCurrentStep) ? 'yellow' : 'transparent';
        drawRoundedRect(ctx, crumb.x, 15, crumb.width, 50, 20, fillColor);
        ctx.fillStyle = 'white';
        ctx.fillText(crumb.label, crumb.x + 15, 40);
        clickableAreas[index] = { x: crumb.x, y: 15, width: crumb.width, height: 50, label: crumb.label, index: index };
        if (index < breadcrumbs.length - 1) {
          drawArrow(ctx, crumb.x + crumb.width, 40);
        }
      });
    }
    function navigateToStep(stepIndex) {
      // Navigate to a previous step based on stepIndex.
      // Update current step.
    ...
    function renderGrid() {
      ...
      container.style.gridTemplateColumns = 'repeat(3, 1fr)'; // maximum 3 per row
      ...
      properties.forEach((property) => {
        ...
        propertyCard.addEventListener('click', () => {
          ...
          renderBreadcrumb(); // Optional: you might not show breadcrumb in grid view.
    ...
    function showPropertyDetail(property) {
      ...
      // (Optionally, you can remove any existing breadcrumb canvas here.)
      ...
      bookViewingLink.addEventListener('click', () => {
        // When booking starts, set booking flow globals.
    ...
    // --------------------------
    // Appointment Scheduler View (Booking Flow Step 1)
    // --------------------------
    function startBookingProcess() {
      container.innerHTML = '';
      container.style.display = 'block';
      // Set booking current step.
      bookingCurrentStep = 1;
      // Insert breadcrumb canvas at top.
      renderBreadcrumb();

      container.innerHTML += `
        <div style="background: white; padding: 20px; text-align: center; max-width: 600px; margin: 20px auto;">
      ...
      // For this view, we simply allow selection from the upcomingDates array.
      // (You can add week navigation if desired.)
    ...
    function renderConfirmationView() {
      ...
      container.innerHTML += `
        <div style="background: white; padding: 20px; text-align: center; max-width: 600px; margin: 20px auto; font-size: 14px;">
      ...
      confirmBtn.addEventListener('click', () => {
        ...
        container.innerHTML = `
          <div style="background: #d4edda; padding: 20px; text-align: center; max-width: 600px; margin: 20px auto; color: #155724; font-size: 14px;">
