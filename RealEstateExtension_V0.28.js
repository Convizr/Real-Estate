export const RealEstateExtension = {
    ...
    render: ({ trace, element }) => {
      ...
      function showPropertyDetail(property) {
        ...
        // Align items so that both the top and bottom are aligned.
        imageRow.style.alignItems = 'stretch';
        imageRow.style.marginBottom = '15px';
        imageRow.style.height = '400px'; // Fixed height
        ...
        // Use space-between so that first and last thumbnails align with top and bottom.
        ...
        // --- Book a Viewing Button with New Styling ---
        ...
        bookViewingButton.style.cssText = `
          ...
          height: 40px;
        `;
        // We'll add a before–like effect using a child span.
        const beforeSpan = document.createElement('span');
        beforeSpan.style.cssText = `
          content: "";
          position: absolute;
          top: -8px;
          left: -8px;
          ...
          z-index: -1;
          mask: conic-gradient(
            transparent 0 calc(105deg - 15deg),
            white calc(105deg - 15deg) calc(105deg + 15deg),
            transparent calc(105deg + 15deg) calc(290deg - 20deg),
            white calc(290deg - 20deg) calc(290deg + 20deg),
            transparent calc(290deg + 20deg)
          );
        `;
        // Because we cannot directly use CSS pseudo–elements in inline styles, we simulate it.
        bookViewingButton.appendChild(beforeSpan);
        // Button text
        const viewingBtnText = document.createElement('span');
        viewingBtnText.textContent = "Book a Viewing";
        viewingBtnText.style.position = 'relative';
        viewingBtnText.style.zIndex = '1';
        bookViewingButton.appendChild(viewingBtnText);
        // Hover effects using CSS variables simulated via JS:
        bookViewingButton.addEventListener('mouseover', () => {
          bookViewingButton.style.transform = 'translateY(-10%) scale(1.1)';
          beforeSpan.style.transform = 'scale(1)';
        });
        bookViewingButton.addEventListener('mouseout', () => {
          bookViewingButton.style.transform = 'translateY(0) scale(1)';
          beforeSpan.style.transform = 'scale(0)';
        });
        bookViewingButton.addEventListener('click', startBookingProcess);
  
        // --- Contact Button with Updated Styling (Blue Outline) ---
        ...
        container.innerHTML = `
          <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.15); text-align: center; max-width: 450px; margin: 20px auto;">
            ...
            <div style="display: flex; justify-content: space-between;">
              <button id="backButton" style="padding: 10px; border: none; border-radius: 8px; background-color: #007BFF; color: white; font-size: 16px; cursor: pointer; transition: background 0.2s; height: 40px;">Back</button>
              <button id="nextButton" style="padding: 10px; border: none; border-radius: 8px; background-color: #007BFF; color: white; font-size: 16px; cursor: pointer; transition: background 0.2s; height: 40px;">Next</button>
            </div>
          </div>
        `;
      ...
      function renderPersonalInfoStep() {
        ...
        container.innerHTML = `
          <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.15); text-align: center; max-width: 450px; margin: 20px auto;">
            ...
            <div style="display: flex; justify-content: space-between;">
              <button id="personalBack" style="padding: 10px; border: none; border-radius: 8px; background-color: #007BFF; color: white; font-size: 16px; cursor: pointer; transition: background 0.2s; height: 40px;">Back</button>
              <button id="personalNext" style="padding: 10px; border: none; border-radius: 8px; background-color: #007BFF; color: white; font-size: 16px; cursor: pointer; transition: background 0.2s; height: 40px;">Next</button>
            </div>
          </div>
        `;
      ...
      function renderConfirmationView() {
        ...
        container.innerHTML = `
          <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.15); text-align: center; max-width: 450px; margin: 20px auto;">
            ...
            <div style="display: flex; justify-content: space-between;">
              <button id="confirmBack" style="padding: 10px; border: none; border-radius: 8px; background-color: #007BFF; color: white; font-size: 16px; cursor: pointer; transition: background 0.2s; height: 40px;">Back</button>
              <button id="confirmBtn" style="padding: 10px; border: none; border-radius: 8px; background-color: #007BFF; color: white; font-size: 16px; cursor: pointer; transition: background 0.2s; height: 40px;">Confirm Viewing</button>
            </div>
          </div>
        `;
        ...
        container.innerHTML = `
          <div style="background: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 8px rgba(0,0,0,0.15); text-align: center; max-width: 450px; margin: 20px auto;">
            ...
            <div style="margin-top: 20px;">
              <button id="contactBack" style="padding: 10px; border: none; border-radius: 8px; background-color: #007BFF; color: white; font-size: 16px; cursor: pointer; transition: background 0.2s; height: 40px;">Back</button>
            </div>
          </div>
        `;
