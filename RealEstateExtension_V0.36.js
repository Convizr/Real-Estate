export const RealEstateExtension = {
    ...
    render: ({ trace, element }) => {
      ...
      // Create a consistent wrapper for sub–views.
      ...
      function showPropertyDetail(property) {
        ...
        imageRow.style.height = '300px'; // Set row height equal to main image container.
      ...
      function startBookingProcess() {
        container.innerHTML = '';
        container.style.display = 'block';
        // Integrated appointment view (no drop shadow, same font, same width)
        container.innerHTML = `
        ...
        // Back button: change text color only.
        backButton.addEventListener('mouseover', () => { backButton.style.color = '#007BFF'; });
        backButton.addEventListener('mouseout', () => { backButton.style.color = 'black'; });
        ...
        // We'll use currentWeekStart based on the first upcoming date.
        ...
        function renderDateButtons() {
          ...
          // Filter upcomingDates to the current week (7 days from currentWeekStart).
          const weekDates = upcomingDates.filter(d => d >= currentWeekStart && d < new Date(currentWeekStart.getTime() + 7 * 86400000));
          // If no dates in current week, clear the date and time selectors.
        ...
        prevWeek.addEventListener('click', () => {
          ...
          updateMonthLabel();
          renderDateButtons();
          timeSlotsContainer.innerHTML = ""; // Clear times when week changes.
        });
        ...
        // Time slots: use defaultTimeSlots if a date is selected.
        let selectedTimeSlot = "";
        function updateTimeSlots(dateStr) {
          timeSlotsContainer.innerHTML = "";
          // If there are no times for the selected date, leave timeSlotsContainer empty.
          // (For demo purposes, assume every selected date gets the defaultTimeSlots.)
          if (!dateStr) return;
        ...
        container.innerHTML = `
          <div style="background: white; padding: 20px; text-align: center; max-width: 450px; margin: 20px auto;">
            ...
            <div style="display: flex; justify-content: space-between;">
              ...
              <button id="personalNext" style="height: 50px; width: 250px; border-radius: 8px; background-color: #007BFF; color: white; padding: 10px; border: none; font-size: 14px; cursor: pointer; transition: background 0.2s;">Next</button>
            </div>
          </div>
        `;
